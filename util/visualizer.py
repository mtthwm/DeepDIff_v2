...
def save_images(webpage, visuals, i, image_path, aspect_ratio=1.0, width=256, use_wandb=False):
    ...
    for label, im_data in visuals.items():
        ...
        im = im_data[0].cpu().float().numpy()
        print(np.max(im))
        print(np.min(im))
        image_name = '%s_%s.png' % (name, label)
        #save_path = os.path.join(image_dir, image_name)
        #util.save_image(im, save_path, aspect_ratio=aspect_ratio)
        util.save_tif(im, image_dir, i, name, label, aspect_ratio=aspect_ratio)
        ...
        if self.display_id > 0:  # show images in the browser using visdom
            ...
            if ncols > 0:        # show all the images in one visdom panel
                ...
                try:
                    ...
                    shapes = [img.shape for img in vis_images]
                    C = 3
                    H = min(s[1] for s in shapes)
                    W = min(s[2] for s in shapes)
                    target_shape = (C, H, W)
                    ...
                    for img in vis_images:
                        if img.shape != target_shape:
                            ...
                            resized = np.zeros(target_shape, dtype=img.dtype)
                            for c in range(C):
                                resized[c] = resize(img[c], (H, W), preserve_range=True)
                            img = resized
                        resized_images.append(img)
    
                    print(f'images shape before vis images reduced size: {[vis_image.shape for vis_image in vis_images]}')
                    self.vis.images(resized_images, nrow=ncols, win=self.display_id + 1,
